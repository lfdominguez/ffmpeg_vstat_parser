...
pub static TSP_RE_CONTINUITY: Lazy<regex::Regex> = Lazy::new(|| {
    regex::Regex::new(
        r"continuity:.+ PID: (?<program_pid>.+), missing (?<missing_count>\d+) packet",
    )
    .unwrap()
});

pub static TSP_RE_HISTORY: Lazy<regex::Regex> =
    Lazy::new(|| regex::Regex::new(r"\* history: (?<packet_number>\d+): (?<message>.+)").unwrap());

pub static TSP_RE_TDT: Lazy<regex::Regex> = Lazy::new(|| {
    regex::Regex::new(r"TDT: (?<datetime>[0-9]{4}/[0-9]{2}/[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2})")
        .unwrap()
});

pub static TSP_RE_PID_MODE: Lazy<regex::Regex> =
    Lazy::new(|| regex::Regex::new(r"PID \d+ \((?<pid>0x\d+)\) (?<action>[^,]+)").unwrap());
